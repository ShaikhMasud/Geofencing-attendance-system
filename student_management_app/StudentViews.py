...
from .models import Subjects, SessionYearModel
...
@login_required
def student_BarcodeScan(request):
    subjects = Subjects.objects.all()
    session_years = SessionYearModel.objects.all()
    return render(request, "student_template/barcode_scanner.html",
                  {"subjects": subjects, "session_years": session_years})


# Define classroom locations for different courses
classroom_locations = {
    "IT": (19.034436, 73.01409),  # Coordinates for IT course
    "Comps": (19.0811433, 72.8890107)  # Coordinates for Comps course
}


def is_inside_geofence(latitude, longitude, coordinates):
    user_location = (latitude, longitude)
    distance = geodesic(user_location, coordinates).meters
    radius = 100  # Define the radius of the geofence
...
def verify_barcode_view(request):
    if request.method == 'POST':
        ...
        print(scanned_barcode,latitude,longitude)
        user = request.user
        try:
            logged_in = CustomUser.objects.get(barcode_number=request.user.barcode_number)
            student = Students.objects.get(admin=user)
            logged_in_user_id = logged_in.barcode_number
        ...
        coordinates = classroom_locations.get(course_name)
        if coordinates is None:
            return JsonResponse({'success': False, 'error': 'Coordinates not found for the course'})

        print(longitude)
        print(latitude)

        if is_inside_geofence(latitude, longitude, coordinates):
            if scanned_barcode == logged_in_user_id:
                # Check if attendance already exists for the same subject and date
                subject_id = data.get("subjectId")
                attendance_date = datetime.datetime.now().date()  # Get current date

                try:
                    subject = Subjects.objects.get(id=subject_id)
                    session_year = SessionYearModel.objects.get(id=student.session_year_id.id)
                    ...
                    if existing_attendance:
                        return JsonResponse(
                            {'success': False, 'error': 'Attendance already marked for this subject and date.'})
                    ...
                    return JsonResponse({'success': False, 'error': 'Subject does not exist.'})
                except SessionYearModel.DoesNotExist:
                    return JsonResponse({'success': False, 'error': 'Session year does not exist.'})
                except Exception as e:
                    return JsonResponse({'success': False, 'error': str(e)})

            else:
                return JsonResponse({'success': False, 'error': 'Barcode does not match with login user'})
        else:
            return JsonResponse({'success': False, 'error': 'User is not within the classroom location'})
...
#     if request.method == 'POST':
#         data = json.loads(request.body)
#         subject_id = data.get("subjectId")  # Get subject ID from the request data
#         attendance_date = data.get("attendance_date")
#         session_year_id = data.get("sessionYearId")
#         try:
#             # Retrieve subject, session year, and create attendance record
#             subject = Subjects.objects.get(id=subject_id)
#             session_year = SessionYearModel.objects.get(id=session_year_id)
#             attendance = Attendance(subject_id=subject, attendance_date=attendance_date, session_year_id=session_year)
#             attendance.save()
#
#             student=Students.objects.get(admin=request.user.id)
#             attendance_report = AttendanceReport(student_id=student, attendance_id=attendance)
...
#         except SessionYearModel.DoesNotExist:
#             return JsonResponse({'status': 'ERROR', 'message': 'Session year does not exist.'})
#         except Exception as e:
